...
def get_parser():
    ...
    aa("--working_dir", type=str, default="/home/qty/code/work")

    aa("--premodel", type=str, default="True",help = "是否使用预训练模型")
    ...
    aa("--encoder_decoderpath",type=str,default="/home/qty/code/water_output/CCNN/fold-0/checkpoint_040.pth")

    aa("--task_model", type=str, default="CCNN",help="检测触发集的分类模型")
    aa("--task_model_path", type=str, default="/home/qty/code/model_ckpt/CCNN_load/train_type_test",help="检测触发集的分类模型地址")
    aa("--identify_model", type=str, default="CCNN",help="检测触发集身份的分类模型")
    aa("--identify_model_path", type=str, default="/home/qty/code/model_ckpt/CCNN_load/train_type_person",help="检测触发集身份的分类模型地址")


    aa("--output_dir", type=str, default="/home/qty/code/water_output/CCNN", help="Output directory for logs and images (Default: /output)")
    ...
    aa("--scaling_i", type=float, default=0.2, help="Scaling of the original image. (Default: 1.0)")

    group = parser.add_argument_group('Optimization parameters')
    aa("--epochs", type=int, default=50, help="Number of epochs for optimization. (Default: 100)")
...
# output_dir = "/home/qty/code/water_output/CCNN"
...
def main(params):
    ...
    working_dir = os.path.join(params.working_dir, "CCNN")
    dataset = get_dataset("CCNN",working_dir ,params.data_path) 
    # 初始化dataloader
    ...
    start_epoch = 0
    folds = 10
    cv = KFold(n_splits=folds, shuffle=True, split_path=f"/home/qty/code/spilt_message/CCNN/{folds}_split")
...
def train_one_epoch(task_model,identify_model, encoder_decoder: models.EncoderDecoder,discriminator: models.Discriminator, loader, optimizer,optimizer_discrim, scheduler, epoch,output_dir, params):
    ...
    for it, (imgs,task_labels,id_labels) in enumerate(metric_logger.log_every(loader, 10, header)):
        ...
        if msgs.is_cuda:
            ...
            numpy_array = tensor_example.numpy().astype('int32')  # 方法1：截断小数
        # 保存水印的数据
