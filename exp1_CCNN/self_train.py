...
class ClassifierTrainer:
    ...
    def fit(self, train_loader,val_loader,pre_loader, epochs, save_path):
            ...
            if (epoch+1) % 5 == 0:
                ...
                for batch_idx, (data, target) in enumerate(pre_loader, start=1):
                    ...
                    if isinstance(target, torch.Tensor):
                        target = target.long()  # 如果已经是Tensor，直接转换类型
                    else:
                        target = torch.from_numpy(target).long()  # 如果是NumPy数组，先转换再改类型
        ...
        return self.graphs.reg_loss,finall_val_accuracy
    ...
    def fit_image(self, train_loader,val_loader, epochs, save_path):
        best_acc = 0.0
        best_epoch = 0
        for epoch in range(epochs):
            self.model.train()
            ...
            for batch_idx, (data, target) in enumerate(train_loader, start=1):
                ...
                for param in self.model.parameters():
                    l2_reg += torch.sum(param**2) 
                num_params = sum(p.numel() for p in self.model.parameters())
                reg_loss = loss_cls + weight_decay * l2_reg / num_params
                reg_loss.backward()
                self.optimizer.step()
                ...
                # 计算准确率
                correct = (predicted == target).sum().item()
            net_correct = correct / len(target)
            
            avg_train_loss = train_loss / len(train_loader)
            self.graphs.reg_loss.append(avg_train_loss)
            print(f"Epoch {epoch}: train_loss={avg_train_loss:.4f}, net_acc={net_correct:.4f}")
            ...
            if (epoch+1) % 10 == 0:
                ...
                if val_accuracy > best_acc:
                    ...
                    best_epoch  = epoch
                    save_path_1 = os.path.join(save_path, f"best_model_{best_epoch}_{net_correct:.4f}_{best_acc:.4f}.ckpt")
                    torch.save(self.model.state_dict(), save_path_1)
                    print(f"Saving model to {save_path_1}")
        return self.graphs.reg_loss, best_acc
